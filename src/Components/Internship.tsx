...
import { InternshipInfo } from '../User.tsx';
...
const TimeLineItem = (items: any) => {
    ...
    return items.map((item: any, index: number) => <Timeline.Item data-aos="fade-up" data-aos-duration="800" key={index}
        className="!pt-12 !mb-2 sm-mx:!p-1" bullet={<IconBriefcaseFilled className='!text-bgColor' size={size} />} >
...
const Internship = () => {
    ...
    return (
        <div className="px-16 mx-20 md-mx:px-6 sm-mx:px-2 lg-mx:mx-0 my-10 mb-28 font-mono" id='Internship'>
            <h1 className="text-4xl sm-mx:text-3xl xs-mx:text-2xl  mb-10 font-bold text-center text-white"><span className='text-primaryColor'>04.&nbsp;</span>Internship</h1>
            <Timeline color='#64FFDA' active={5} bulletSize={dot} lineWidth={2}>
                {
                    TimeLineItem(InternshipInfo)
                }
